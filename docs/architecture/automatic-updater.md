...
앱 내부 업데이트는 패키지를 검사한 뒤 설치 폴더의 `.update-tmp/staged`에 준비한다. 설치 폴더 밖으로 복사한 Node와 `server/node/portable-update.cjs`가 기존 서버 PID의 종료를 확인한 후 파일을 교체한다. 실행 중인 `RisuBard.exe`는 유지하고, 사용자 인증서는 준비한 패키지에 복사한다. 새 서버의 listen 완료를 확인한 뒤 버전을 확정하고 백업을 정리한다. 기동 실패 시 앱 파일만 복구하며, 복구 실패의 백업과 journal은 남긴다. 후처리 기록은 설치 루트 `update.log`에 남는다.

설치 전후 검사는 부모 폴더의 의존성을 이용하지 않도록 패키지 내부 의존성 manifest와 주요 서버 모듈 로딩을 확인한다. 독립 업데이터도 같은 검사를 사용하며, 같은 버전이라도 설치가 불완전하면 다시 다운로드한다. Windows 독립 업데이트 중 실행 중인 `update.bat`은 교체하지 않는다.

0.9.34 이하에서 0.9.35를 받는 작업은 구버전 업데이터가 수행한다. 첫 전환은 서버를 종료하고 ZIP을 새 폴더에 풀어 사용자 데이터를 복사하는 경로를 권장한다. 릴리즈 CI는 최종 압축 파일을 다시 풀어 동봉된 Node로 서버를 실행하며, 테스트 저장 데이터는 배포 폴더 밖에 둔다.
