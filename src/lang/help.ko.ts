...
export const helpKo = {
        ...
        "chatUploadChunkSize": "채팅 저장 시 한 번에 보낼 데이터 크기입니다. 기본 8MiB, 설정 범위는 1~64MiB입니다. 변경하면 다음 채팅 저장부터 앱과 서버에 함께 적용되며 재시작할 필요가 없습니다. 작게 설정할수록 요청 횟수가 늘어납니다.",
        "chatUploadChunkEnabled": "기본값은 꺼짐으로 기존 저장 방식을 유지합니다. 켜면 설정한 크기보다 큰 채팅을 나눠 업로드합니다. 끄면 채팅 전체를 한 번에 전송하므로 서버나 프록시의 요청 크기 제한에 걸릴 수 있습니다. 변경은 다음 저장부터 적용됩니다.",
        ...
        "risuBardRecentMessages": "BardWiki 분석에 문맥으로 추가할 최근 어시스턴트 턴 수입니다. 사용자 메시지는 ‘분석 사용자 메시지’ 설정을 따릅니다.",
        "risuBardResponseRecentMessages": "새 답변을 만들 때 참조할 최근 어시스턴트 턴 수입니다. 현재 사용자 요청은 별도로 항상 포함합니다.",
        ...
        "risuBardBardChanEnabled": "후보 점수가 비슷해 검색 결과가 애매할 때만 선택한 모델이 짧은 후보 카드의 순서를 한 번 보정합니다. 기본값은 꺼짐이며, 모델 호출에 실패하면 기존 검색 결과를 그대로 사용합니다.",
        "risuBardBardChanModelMode": "바드쨩의 재순위 요청에 사용할 모델을 고릅니다. 보조 모델은 비용을 아끼기 좋고, 메인 모델은 애매한 후보를 더 정확히 가려야 할 때 유용합니다.",
        ...
        "autoTranslation": "캐릭터 메시지 자동 번역의 기본값입니다. 입력창의 자동 번역 버튼으로 현재 채팅에만 별도 설정을 적용할 수 있으며, 별도 설정이 없는 채팅은 이 기본값을 따릅니다. 사용자 메시지와 입력 내용은 자동 번역하지 않습니다. 메시지별 번역 버튼이나 입력창 번역하기를 사용하세요. \"번역 대기 안 함\" 옵션과 함께 쓰면 원문이 먼저 보이고 번역이 추후 추가됩니다.",
