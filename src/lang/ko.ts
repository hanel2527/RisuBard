...
export const languageKorean = {
  ...
  lorebookWorkspace: {
    ...
    bardLoreDescription: "레거시 원본은 보존하고, 고정 예산 검색으로 선택된 항목만 생성에 주입합니다.",
    ...
    bardAnalysisSettings: "AI 분석 제한",
    bardAnalysisBatchEntries: "분석 배치당 항목 수",
    bardAnalysisInputTokens: "분석 입력 토큰 제한",
    bardAnalysisOutputTokens: "분석 출력 토큰 제한",
    bardAnalysisLinkedDepth: "연결 항목 분석 깊이",
    bardAnalysisTemperature: "분석 temperature",
    bardAnalysisSettingsHint: "이 캐릭터에 저장됩니다. 숫자 입력에는 앱이 숨은 최대값을 적용하지 않습니다.",
    ...
    bardActivationHelp: "이 항목을 항상 필수로 넣을지, 키·별칭 적중이나 관련도 검색으로 선택할지, 자동 주입에서 제외할지 정합니다.",
    bardRequired: "항상 필수",
    bardKeyed: "키·별칭 적중",
    bardRetrieve: "관련도 검색",
    bardNever: "자동 주입 안 함",
    ...
    bardGuideIntro: "그리모어는 원본 로어북을 그대로 보존하면서 고정 예산 안에서 필요한 로어만 선택합니다. 생성 중 검색은 로컬에서 실행되며 AI 모델을 추가로 호출하지 않습니다.",
    ...
    bardGuideRetrieveBody: "항목 메타데이터와 본문의 관련도 검색 점수가 설정한 기준을 넘을 때 선택됩니다.",
    ...
    bardGuideCompatibilityBody: "레거시 로어북은 호환 원본으로 계속 보존되고 그리모어 편집이 이를 덮어쓰지 않습니다. 메타데이터 가져오기·내보내기는 그리모어 오버레이만 옮기며, 정화도 레거시 로어북을 남기고 오버레이만 제거합니다.",
    bardKind: "로어 유형",
    ...
    bardAnalysisEstimateNotice: (count: number) => `모델 요청 ${count}회를 실행합니다. 입력 견적은 실제 배치 프롬프트 기준이며 공급자 과금량과는 차이가 날 수 있습니다.`,
