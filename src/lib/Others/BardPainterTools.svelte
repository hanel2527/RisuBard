            ...
            {:else if mode === 'settings'}
                <fieldset disabled={locked}>
                    <section aria-label="AI에 보낼 참고 자료">
                        ...
                        <div class="grid">
                            <label>이전 메시지 수<input aria-label="이전 메시지 수" type="number" min="0" max="20" step="1" bind:value={data.settings.context.before} onchange={save} /></label>
                            <label>이후 메시지 수<input aria-label="이후 메시지 수" type="number" min="0" max="20" step="1" bind:value={data.settings.context.after} onchange={save} /></label>
                        </div>
                        <div class="checks">
                            <label class="check"><input type="checkbox" bind:checked={data.settings.context.surrounding} onchange={save} />선택한 메시지의 나머지 내용</label>
                            <label class="check"><input type="checkbox" bind:checked={data.settings.context.characterDescription} onchange={save} />캐릭터 설명</label>
                            <label class="check"><input type="checkbox" bind:checked={data.settings.context.persona} onchange={save} />페르소나</label>
                            <label class="check"><input type="checkbox" bind:checked={data.settings.context.systemPrompt} onchange={save} />채팅 시스템 프롬프트</label>
                            <label class="check"><input type="checkbox" bind:checked={data.settings.context.characterLorebook} onchange={save} />캐릭터 로어북</label>
                            <label class="check"><input type="checkbox" bind:checked={data.settings.context.moduleLorebook} onchange={save} />모듈 로어북</label>
                        </div>
                        <details class="advanced">
                            ...
                            <div class="actions">
                                ...
                                {#if data.settings.context.wikiIds.length}<button type="button" onclick={() => { data.settings.context.wikiIds = []; save() }}>선택 해제</button>{/if}
                            </div>
                            <div class="wiki-list">
                        ...
                        <label>프롬프트 작성 모델<select bind:value={data.settings.modelSlot} onchange={save}><option value="model">메인 모델</option><option value="submodel">보조 모델</option></select></label>
                    </section>
                    <section aria-label="이미지 생성 설정">
                        <h3>이미지 생성</h3>
                        <label>NovelAI 모델<select bind:value={data.settings.model} onchange={save}><option value="nai-diffusion-5-full">NovelAI V5 Full</option><option value="nai-diffusion-5-curated">NovelAI V5 Curated</option></select></label>
                        <div class="grid">
                            <label>이미지 크기<select aria-label="이미지 크기" value={`${data.settings.width}x${data.settings.height}`} onchange={event => { const [width, height] = event.currentTarget.value.split('x').map(Number); data.settings.width = width; data.settings.height = height; save() }}><option value="832x1216">세로 832 × 1216</option><option value="1216x832">가로 1216 × 832</option><option value="1024x1024">정사각 1024 × 1024</option></select></label>
                            <label>시드<span class="hint">비우면 무작위</span><input aria-label="이미지 시드" type="number" min="0" max="4294967295" step="1" value={data.settings.seed ?? ''} onchange={event => { data.settings.seed = event.currentTarget.value === '' ? null : event.currentTarget.valueAsNumber; save() }} /></label>
                        </div>
