<script lang="ts">
    ...
    async function run(action: 'status' | 'migrate' | 'refresh' | 'rollback') {
        ...
        try {
            ...
            message = 'V3 전환 상태를 확인하지 못했습니다. 저장이 끝난 뒤 다시 시도해 주세요. 복구 가능한 원본은 유지됩니다.'
        } finally { busy = false }
    }
</script>
...
{#if isNodeServer}
    <section class="asset-pilot">
        ...
        <div class="actions">
            ...
            <ShButton variant="outline" onclick={() => run('rollback')} disabled={!characterId || busy || result?.enabled === false}>기존 구조로 되돌리기</ShButton>
        </div>
        <div role="status" aria-live="polite">
            {#if result}
                <p>{result.enabled ? `V3 구조 사용 중: ${result.directory} 폴더, 채팅 ${result.chats}개` : '기존 ID 폴더와 KV 에셋 읽기를 사용합니다.'}</p>
