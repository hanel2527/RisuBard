...
export function registerCBS(arg:CBSRegisterArg) {
    ...
    registerFunction({
        ...
        callback: (str, matcherArg, args, vars) => {
            const db = getDatabase()
            const selchar = matcherArg.chara && typeof matcherArg.chara !== 'string' && 'chats' in matcherArg.chara
                ? matcherArg.chara
                : db.characters[getSelectedCharID()]
            if(!selchar){
                return ''
            }
            const chat = selchar.chats[selchar.chatPage]
    ...
    registerFunction({
        name: 'lastmessageid',
        callback: (str, matcherArg, args, vars) => {
            if(matcherArg.cbsConditions.lastMessageId !== undefined){
                return matcherArg.cbsConditions.lastMessageId.toString()
            }
            const db = getDatabase()
            const selchar = matcherArg.chara && typeof matcherArg.chara !== 'string' && 'chats' in matcherArg.chara
                ? matcherArg.chara
    ...
    registerFunction({
        ...
        callback: (str, matcherArg, args, vars) => {
            const db = getDatabase()
            const selchar = matcherArg.chara && typeof matcherArg.chara !== 'string'
                ? matcherArg.chara
                : db.characters[getSelectedCharID()]
            if(!selchar){
                return ''
