...
describe('Bard Lore portable metadata overlay', () => {
    it('exports approved metadata without serializing ordinary legacy lore bodies', () => {
        const state = upgrade([legacy('school', '월광관 고등학교', '공유하면 안 되는 학교 전체 본문')])
        state.entries[0].bard = {
            ...state.entries[0].bard,
            kind: 'location',
            aliases: ['학교'],
            tags: ['교육기관'],
        ...
        const exported = exportBardLoreMetadata(state, '페르소나 3')
    ...
    it('imports metadata and remaps links by source fingerprint when target IDs differ', () => {
        ...
        source.entries[0].bard.kind = 'location'
        source.entries[1].bard = {
            ...source.entries[1].bard,
            kind: 'character',
            aliases: ['슈지'],
        ...
        const result = importBardLoreMetadata(target, exportBardLoreMetadata(source), () => 'unused')

        expect(target).toEqual(before)
        expect(result.report).toMatchObject({ applied: 2, createdDerived: 0, skipped: 0, unresolvedLinks: 0 })
        expect(result.state.entries.map((entry) => entry.id)).toEqual(['school-target', 'shuji-target'])
        expect(result.state.entries[1].bard).toMatchObject({
            kind: 'character',
            aliases: ['슈지'],
    ...
    it('reconstructs derived analysis atoms without exporting the composite source body', () => {
        const source = upgrade([legacy('roster', '주요 인물 명부', '긴 명부 본문\n슈지: 학교 이사장.')])
        ...
        source.entries.push(atom)
        const exported = exportBardLoreMetadata(source)
        const target = upgrade([legacy('roster-copy', '주요 인물 명부', '긴 명부 본문\n슈지: 학교 이사장.')])

        const result = importBardLoreMetadata(target, exported, () => 'imported-shuji')
        const derived = result.state.entries.find((entry): entry is BardLoreEntry => entry.id === 'bard-atom-shuji')
    ...
    it('rejects malformed packages without mutating the current Bard state', () => {
        ...
        expect(() => importBardLoreMetadata(state, '{"type":"wrong"}', () => 'new')).toThrow(BardLorePortableError)
    ...
    it('never maps two portable entries onto the same destination entry', () => {
        ...
        source.entries[0].bard.summary = '첫 번째 메타데이터'
        source.entries[1].bard.summary = '두 번째 메타데이터'
        const target = upgrade([legacy('only-target', '같은 항목', '동일 본문')])

        const result = importBardLoreMetadata(target, exportBardLoreMetadata(source), () => 'unused')

        expect(result.report).toMatchObject({ applied: 1, skipped: 1 })
        expect(result.state.entries[0].bard.summary).toBe('첫 번째 메타데이터')
    })

    it('preserves an analysis draft when no portable entry matches', () => {
        const source = upgrade([legacy('source', 'Source', 'source content')])
        const target = upgrade([legacy('target', 'Target', 'different content')])
