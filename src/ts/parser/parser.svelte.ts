...
async function parseAdditionalAssets(data:string, char:simpleCharacterArgument|character, mode:'normal'|'back', arg:{ch:number}){
    ...
    if (char.type === 'character' && (!assetsCache || !emoAssetsCache || assetsCacheCharacterId !== char.chaId)) {
        ...
        if(p1.startsWith('#when ')){
            ...
            while(statement.length > 1){
                ...
                switch(operator){
                    ...
                    case 'toggle':{
                        const variable = getScopedGlobalChatVar('toggle_' + condition)
                        if(isTruthy(variable)){
                            statement.push('1')
                        }
                        else{
                    ...
                    case 'tis':{ //tis = toggle is
                        const variable = getScopedGlobalChatVar('toggle_' + statement.pop())
                        if(variable === condition){
                            statement.push('1')
                        }
                        else{
