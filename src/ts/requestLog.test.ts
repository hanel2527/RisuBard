...
describe('createRequestLogScope', () => {
    ...
    it('records the owning chat and the request-status injection manifest', async () => {
        const scope = createRequestLogScope({
            category: 'llm',
            source: 'main',
            chatId: 'generation-1',
        ...
        const [entry] = posted[0]
        expect(entry.sessionChatId).toBe('chat-1')
        expect(entry.injectionManifest).toEqual({
            totalTokens: 130,
            estimated: true,
            items: [
                { kind: 'systemPrompt', tokens: 20 },
    ...
    it('applies usage only to the last request of a tool loop', async () => {
        ...
        scope.setInjectionManifest({
            totalTokens: 5,
            items: [{ kind: 'chatHistory', tokens: 5 }],
        })
        const wrapped = scope.wrap(async () => jsonResponse('{}'))
        await (await wrapped('https://x.test/1', { method: 'POST', body: '{}' })).text()
        await (await wrapped('https://x.test/2', { method: 'POST', body: '{}' })).text()
        ...
        expect(posted[0][0].injectionManifest).toBeUndefined()
        expect(posted[0][1].inputTokens).toBe(5)
        expect(posted[0][1].injectionManifest?.totalTokens).toBe(5)
    })
