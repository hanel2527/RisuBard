...
export type RequestLogSource =
    ...
    | 'wiki-admin' | 'preview' | 'test'
    | 'tts' | 'image' | 'plugin' | 'other'
...
function toQueryString(query: RequestLogQuery): string {
    ...
    if (query.sessionChatId) params.set('session_chat_id', query.sessionChatId)
...
export interface RequestLogScope {
    ...
    /** Body-free request-status breakdown stored on the first provider request. */
    setInjectionManifest(manifest: RequestInjectionManifest | undefined): void
...
export function createRequestLogScope(init: RequestLogScopeInit): RequestLogScope {
    ...
    return {
        ...
        setUsage(usage) {
            ...
            if (!last) return
            // Tool/fallback loops can issue more than one provider request in
            // one status scope. The terminal card describes the last request,
            // whose usage is authoritative, so keep its manifest on that row.
            const manifested = entries.find((entry) => entry.injectionManifest)
            if (manifested && manifested !== last) {
                last.injectionManifest = manifested.injectionManifest
                delete manifested.injectionManifest
            }
        ...
        setInjectionManifest(manifest) {
            injectionManifest = manifest
            if (entries[0]) entries[0].injectionManifest = manifest
        },
        setModel(model) {
            if (!model) return
